...
def bfs():
    global ans, tablet
    queue = deque()
    queue.append(('B', tablet, 0, 0, n*3 - 1))
    while queue:
        now, tablet, len, start, end = queue.popleft()
        ans = max(ans, len)
        if start + 1 <= n * 3 - 1 and not visited[start+1][end] and tablet and tablet[0] == now:
            new_tablet = copy.deepcopy(tablet)
            new_tablet.popleft()
            visited[start+1][end] = True
            queue.append((template[now], new_tablet, len + 1, start+1, end))
        if end - 1 >= 0 and not visited[start][end-1] and tablet and tablet[-1] == now:
            new_tablet = copy.deepcopy(tablet)
            new_tablet.pop()
